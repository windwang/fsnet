...
public class PreventIncomingEventsJob implements Job{
	...
	private void preventIncomingEvents(){
		...
		if(incomingEvents!=null){
			...
			while(iterator.hasNext()) {       
				...
				for(SocialEntity socialEntity : (Set<SocialEntity>)incomingEvents.get(meeting)){
					sendPreventIncomingEventMail(meeting,socialEntity,incomingEvents.get(meeting));
				}
			}
		}
	}
	...
	private void sendPreventIncomingEventMail(Meeting meeting,SocialEntity socialEntity,
			Set<SocialEntity> listSubscribes) {
		...
		message = createMessage(meeting,socialEntity,fsnetAddress,listSubscribes);
		...
		mail.setSubject(bundle.getMessage("events.recallMail.subject.recall") + " - " +
				meeting.getTitle() + " " + bundle.getMessage("events.recallMail.subject.today"));
	...
	private String createMessage(Meeting meeting, SocialEntity entity,String fsnetAddress,
			Set<SocialEntity> listSubscribes) {
		MessageResources bundle = MessageResources
				.getMessageResources("FSneti18n");
		
		String message = "<h1>" + bundle.getMessage("events.recallMail.title") + "</h1>";
		message += "<p>" + meeting.getTitle() + "</p>";
		message += "<ul>";
		message += "<li>" + bundle.getMessage("events.recallMail.startDate") + ": " +
				DateUtils.renderDBDate(meeting.getStartDate()) + "</li>";
		...
		message += "</li>";
		message += "<li>"+ bundle.getMessage("events.recallMail.fsnet") + ": " + fsnetAddress + "</li>";
		message += "</ul>";
