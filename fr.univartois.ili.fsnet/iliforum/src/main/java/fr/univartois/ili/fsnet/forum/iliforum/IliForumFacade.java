package fr.univartois.ili.fsnet.forum.iliforum;
...
import fr.univartois.ili.fsnet.forum.ForumFacade;
...
public final class IliForumFacade implements ForumFacade {
    ...
    public List<Hub> getListHub(final Date dateBegin, final Date dateEnd) {
        Query query;
        query = entM.createQuery("SELECT h FROM Hub h WHERE h.dateCreation>?1 and h.dateCreation<?2 ORDER BY h.dateCreation");
    ...
    public List<Hub> getListHubByEntiteSociale(final EntiteSociale decideur) {
        Query query;
        query = entM.createQuery("SELECT h FROM Hub h WHERE h.decideur =?1 ORDER BY h.dateCreation");
    ...
    public List<Message> getListMessage() {
        Query query;
        query = entM.createQuery("SELECT m FROM Message m ORDER BY m.dateMessage");
    ...
    public List<Message> getListMessage(final Date dateBegin, final Date dateEnd) {
        Query query;
        query = entM.createQuery("SELECT m FROM Message m WHERE m.dateMessage>?1 AND m.dateMessage<?2 ORDER BY m.dateMessage");
        query.setParameter(1, dateBegin);
        query.setParameter(2, dateEnd);
        List<Message> mesMess;
        mesMess = (List<Message>) query.getResultList();
    ...
    public List<Message> getListMessageByEntiteSocial(
        ...
        Query query;
        query = entM.createQuery("SELECT m FROM Message m WHERE m.propMsg=?1 ORDER BY m.dateMessage");
    ...
    public List<Message> getListMessageByHub(final Hub hub) {
        Query query;
        query = entM.createQuery("SELECT m FROM Message m WHERE m.topic.hub=?1 ORDER BY m.dateMessage");
    ...
    public List<Message> getListMessageByTopic(final Topic topic) {
        Query query;
        query = entM.createQuery("SELECT m FROM Message m WHERE m.topic=?1 ORDER BY m.dateMessage");
    ...
    public List<Topic> getListTopic(final Date dateBegin, final Date dateEnd) {
        Query query;
        query = entM.createQuery("SELECT t FROM Topic t WHERE t.dateSujet>?1 AND t.dateSujet<?2 ORDER BY t.dateSujet");
        query.setParameter(1, dateBegin);
        query.setParameter(2, dateEnd);
        List<Topic> mesTopics;
        mesTopics = (List<Topic>) query.getResultList();
    ...
    public List<Topic> getListTopicByEntiteSociale(
        ...
        Query query;
        query = entM.createQuery("SELECT t FROM Topic t WHERE t.propTopic=?1 ORDER BY t.dateSujet");
    ...
    public List<Topic> getListTopicByHub(final Hub hub) {
        Query query;
        query = entM.createQuery("SELECT t FROM Topic t WHERE t.hub=?1 ORDER BY t.dateSujet");
        query.setParameter(1, hub);
        List<Topic> mesTopics;
        mesTopics = (List<Topic>) query.getResultList();
        return mesTopics;
