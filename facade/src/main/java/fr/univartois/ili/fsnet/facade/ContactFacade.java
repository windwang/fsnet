...
public class ContactFacade {
	...
	public final void askContact(SocialEntity asker, SocialEntity asked) {
		...
		if (!(asker.getAsked().contains(asked)
				|| asker.getRequested().contains(asked) || asker.getContacts()
				.contains(asked))) {
			asker.getRequested().add(asked);
			asked.getAsked().add(asker);
		} else if (!(asked.getAsked().contains(asker)
				|| asked.getRequested().contains(asker) || asked.getContacts()
				.contains(asker))) {
			asker.getRequested().add(asked);
			asked.getAsked().add(asker);
		} else {
	 ...
	 * @param removedEntity
	 */
	public final void removeContact(SocialEntity member,
			SocialEntity removedEntity) {
	...
	public final void cancelRequested(SocialEntity user,
			SocialEntity requested, EntityManager em) {
