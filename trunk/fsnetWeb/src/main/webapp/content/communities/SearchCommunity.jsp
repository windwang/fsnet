...
<html:form action="SearchCommunity" method="GET">
	<div id="SearchCommunity"><html:text property="searchText" /> <html:submit
		styleClass="button">
...
<c:choose>
	<c:when
		test="${! empty requestScope.communitiesSearchPaginator.resultList}">
		<table class="inLineTable">
			<c:forEach var="community"
				items="${requestScope.communitiesSearchPaginator.resultList}">
				<tr class="content">
					<td>
					    <c:import url="/FavoriteFragment.do">
						<c:param name="interactionId" value="${community.id}" />
					</c:import> <html:link action="/DisplayCommunity" title="${empty community.interests?\"\":community.interests}">
						<html:param name="communityId" value="${community.id}" />
                            ${community.title}
                        </html:link></td>
					<td><bean:message key="communities.by" /> <ili:getSocialEntityInfos
		...
		<c:set var="paginatorInstance"
			value="${requestScope.communitiesSearchPaginator}" scope="request" />
